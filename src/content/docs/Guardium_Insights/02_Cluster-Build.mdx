...
Download (Save link as...) the customized [crds.yaml](/ibm-client-engineering/engineering-journal-quantum-safe/olm/crds.yaml) and [olm.yaml](/ibm-client-engineering/engineering-journal-quantum-safe/olm/olm.yaml) for OLM v0.28.0. Keep in mind, this is a point in time customization for GI 3.5.0 and
OLM v0.28.0.
...
Find the private cluster IP address of the load balancer under `CLUSTER-IP`.
...
We want to create a record that routes `cp-console-openshift-marketplace.apps.gi.guardium-insights.com` to
the private IP address captured above.

![image](https://media.github.ibm.com/user/207188/files/fbb0bb48-1795-4fbb-af74-c068ecb562b2)
...
oc ibm-pak launch $CASE_NAME \
   ...
   --inventory $ICS_INVENTORY_SETUP \
   --namespace $NAMESPACE \
   --args "--size ${ICS_SIZE} --ks_flag true --hostname ${HOSTNAME} --registry cp.icr.io --user ${CP_REPO_USER} --pass ${CP_REPO_PASS} --secret ibm-entitlement-key --recursive --inputDir ${LOCAL_CASE_DIR}"
```

:::note
It seems sometimes the above command will fail waiting for the `platform-oidc-credentials` to be created. It might mean that the common-services operator needs to be bounced or that you might need to wait between running commands. If it fails on that error, run the following:

```tsx
kubectl scale deploy ibm-common-service-operator --replicas=0
kubectl scale deploy ibm-common-service-operator --replicas=1
```

Then re-run the `oc ibm-pak launch` command once the operator is back up.
:::
