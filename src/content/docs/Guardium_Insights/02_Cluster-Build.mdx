...
As of this writing, the latest version of AWS Load Balancer Controller is [v2.8.3](https://github.com/kubernetes-sigs/aws-load-balancer-controller/releases/tag/v2.8.3)
...
{/*arn:aws:iam::803455550593:policy/AWSLoadBalancerControllerIAMPolicy*/}
...
```
{/*
While we're at it, let's retrieve the arn of our eks nodes role. In our case the role name is `gi-eks-nodes-role`. This may be different for you.

export node_role_arn=$(aws iam list-roles --query 'Roles[?RoleName==`gi-eks-nodes-role`].Arn' --output text)
