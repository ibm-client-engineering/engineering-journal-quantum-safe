...
gi-eks   Ready   True     Complete   Completed Reconciliation   3.4.0             3.4.0
```
...
```tsx {2}
NAME                           TYPE    STATUS   REASON     MESSAGE                    DESIRED_VERSION   INSTALLED_VERSION
gi-eksctqdrippjy9p2gqvvzwhyc   Ready   True     Complete   Completed Reconciliation   3.4.0             3.4.0
```
...
Create records in the same way we did previously, only this time for the remaining ingresses that
were created by the GI deployment. 

Using the sample output above, this would translate to records for the following hostnames resolving to the private cluster IP.

```
s3.apps.gi.guardium-insights.com
feed-ctqdrippjy9p2gqvvzwhyc-gi-eks.apps.gi.guardium-insights.com
unix-ctqdrippjy9p2gqvvzwhyc-gi-eks.apps.gi.guardium-insights.com
```
:::
