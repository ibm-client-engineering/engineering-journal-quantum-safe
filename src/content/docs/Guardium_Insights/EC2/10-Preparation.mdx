...
```
aws cloudformation deploy --stack-name stack-deployment-1 --template-file cluster.yml --parameter-overrides file://parameters-override.yaml --capabilities CAPABILITY_NAMED_IAM --tags *add Key=Value tag here* --role-arn arn:aws:iam::<ACCOUNT>:role/OCPInstall
```
