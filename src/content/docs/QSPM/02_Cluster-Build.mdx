...
```
Take note of the `arn` entry for the policy that is returned above as you will need it below. From the example output above, our `arn` would be `arn:aws:iam::<ACCOUNT ID>:policy/AmazonEKS_EBS_CSI_Driver_Policy` where `<ACCOUNT ID>` would be our actual AWS account id.
...
```
Take note of the `arn` entry for the role that is returned above as you will need it below. In the below example the `arn` returned from above would be `arn:aws:iam::<ACCOUNT ID>:role/AmazonEKS_EBS_CSI_DriverRole`
...
kube-system	aws-load-balancer-controller	arn:aws:iam::<ACCOUNT ID>:role/AmazonEKSLoadBalancerControllerRole
kube-system	efs-csi-controller-sa		arn:aws:iam::<ACCOUNT ID>:role/eksctl-guardium-east-addon-iamserviceaccount-ku-Role1-1SCBRU1DS52QY
```
...
Make sure the filesystem id you use in the below command is the filesystem id that was returned to you above!

:::

Update it with the storage class id.
```bash
sed -i 's/fileSystemId:.*/fileSystemId: <RETURNED FS-ID>/' EFSStorageClass.yaml
```
