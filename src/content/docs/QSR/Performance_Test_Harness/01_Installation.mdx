...
- `TESTFQDN` - This can be the local IP of the docker host we are running the tests from.

```tsx
./build_config.sh

Enter the target FQDN list seperated by space (TARGET_FQDN_LIST): 9.46.99.87
Enter the target IP list seperated by space (TARGET_IP_LIST): 9.46.99.87
