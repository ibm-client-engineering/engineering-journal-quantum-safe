...
We are adding three location blocks for the three directory paths here. And we're reserving the `/app/` path for the root page of the proxied site. This does break things somewhat, but by doing this we are leaving `/` open for the adaptive proxy landing page.
...
location ^~ /guides/example/ {
    proxy_pass http://apache-host01.fyre.ibm.com:8080/guides/example/;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto $scheme;
...
location ^~ /reference/example/ {
    proxy_pass http://apache-host01.fyre.ibm.com:8080/reference/example/;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Forwarded-Proto $scheme;
...
We can also verify that the Adaptive Proxy landing page is also available:

![CleanShot 2024-07-16 at 13 36 23@2x](https://media.github.ibm.com/user/20330/files/e4c5c254-3283-4977-90b4-9cfa797d22e0)
